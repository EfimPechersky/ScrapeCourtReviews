        ...
        if int(review_count.text)>50:
            ...
            while True:
                ...
                try:
                    ...
                    try:
                        browser.find_element(By.XPATH, '//div[contains(@aria-posinset, "'+str(int(review_count.text)-1)+'")]')
...
@profile
def main():
    cService = wd.ChromeService(executable_path='C:/Users/EfimPechersky/AppData/Local/Programs/Python/Python312/chromedriver/chromedriver.exe')
    browser = wd.Chrome(service = cService)
    browser.get("https://yandex.ru/maps/")
    start=int(input())
    end=int(input())
    all_courts={}
    with open("../gas/ms_names.txt") as file:
        ...
        for i in names[start:end]:
            ...
            print(count+start-1)
    browser.quit()
    with open("yandex_ms_data"+str(start)+str(end)+".json","w") as file:
